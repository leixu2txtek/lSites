...
define(['jquery', 'template', 'pace',], function ($, template) {
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: { siteId: util.get_query('siteId') },
        async: false
    }).done(function (r) {
        ...
        //设置用户信息
        var info = $(template.compile('<div class="navbar-collapse collapsc navbar-right group-top-userbox"><span>欢迎您，{{display_name}}</span> <a href="javascript:(0);" class="group-top-user user_info"><img src="{{avatar}}" class="img-circle" width="38px" height="38px"> <i class="glyphicon glyphicon-chevron-down"></i></a><div class="nav group-user-dropdown" style="display:none"><ul><i class="group-user-dropdownIcon">&nbsp;</i><li><a href="#" title="个人资料">个人资料</a></li><li><a href="#" title="修改资料">修改资料</a></li><li><a href="/users/logout" title="退出">退出</a></li></ul></div></div>')({
        ...
        $('.user_info', info).on('click', function () {
            ...
            $('body').one('click', function () { next.hide(300).data('open', false); });
